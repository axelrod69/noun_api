        ...
        if (err) {
            ...
            DB.query(sql, [phone], async (err, result) => {
                ...
                if (result.length == 1) {
                    ...
                    DB.query(sql, [phone, otp, otpExpiredAt], async (err, result) => {
                        if (err) {
                            return res.status(200).json({
                                status: 'warning',
                                message: 'Something went wrong',
...
exports.addStation = async (req, res) => {
    DB.getConnection(async (err, connection) => {
        ...
        if (err) {
            ...
            let test = DB.query(sql, [name, phone, email, OTP, otpExpiredAt], (err, result) => {
                if (err) {
                    return res
                        .status(200)
                        .json({
...
exports.bookingList = async (req, res) => {
    let stationKey = req.jwtUser['userId'];
    let sql = "SELECT `booking_details`.*,`car_details`.`carBrand`,`car_details`.`carModel`,`car_details`.`carSubModel`,`car_details`.`carImage` FROM `booking_details`,`car_details` WHERE `booking_details`.`customerCarKey`=`car_details`.`carKey` AND `booking_details`.`stationKey`=?";
    DB.query(sql, [stationKey], async (err, result) => {
        if (err) {
            return res
                .status(200)
...
exports.bookingDetails = async (req, res) => {
    ...
    let sql = "SELECT `booking_details`.*,`charging_ports`.`portName`,`charging_ports`.`portDesc`,CONCAT('http://localhost:2060/public/storage/docs/',`charging_ports`.`portImage`) AS 'portImage',`charging_ports`.`portType`,`car_details`.`carBrand`,`car_details`.`carModel`,`car_details`.`carSubModel`,`car_details`.`carImage`, `users`.`name` AS 'customerName', `users`.`email` AS 'customerEmail', `users`.`phone` AS 'customerPhone' FROM `booking_details`,`car_details`,`charging_ports`, `users` WHERE `booking_details`.`customerCarKey`=`car_details`.`carKey` AND `booking_details`.`stationPortKey`=`charging_ports`.`portKey` AND `booking_details`.`customerKey`=`users`.`id` AND `booking_details`.`stationKey`=? AND `booking_details`.`bookingKey`=?";
